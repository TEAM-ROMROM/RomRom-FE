...
import 'package:romrom_fe/screens/item_price_screen.dart';
...
class _RegisterInputFormState extends State<RegisterInputForm> {
  ...
  bool _isAiRecommendedPrice = false; // AI 추천 가격 여부 (칩 표시 조건)
  bool _isPriceScreenOpen = false; // 가격 페이지 중복 진입 방지
  ...
  Future<void> _initControllers() async {
    ...
    if (widget.isEditMode && widget.item != null) {
      ...
      // AI 가격 토글은 ItemPriceScreen에서 처리됨
    }
  }
              ...
              children: [
                ...
                RegisterCustomLabeledField(
                  ...
                  field: GestureDetector(
                    onTap: () async {
                      if (_isPriceScreenOpen) return;
                      _isPriceScreenOpen = true;
                      final currentPriceText = priceController.text.replaceAll(',', '').trim();
                      final currentPrice = int.tryParse(currentPriceText) ?? 0;
                      ...
                      if (result != null && result.$1 > 0 && mounted) {
                        final formatted = const PriceCommaFormatter().formatEditUpdate(
                          const TextEditingValue(),
                          TextEditingValue(text: result.$1.toString()),
                        );
                        setState(() {
                          priceController.value = formatted;
                          _isAiRecommendedPrice = result.$2;
                        });
                      }
                      _isPriceScreenOpen = false;
                    },
                    child: Container(
                      padding: EdgeInsets.symmetric(horizontal: 16.w, vertical: 14.h),
                      decoration: BoxDecoration(
                      ...
                      child: Row(
                        children: [
                          ...
                          const Spacer(),
                          // AI 추천 가격일 때만 칩 표시
                          if (_isAiRecommendedPrice) ...[const AiPriceChipWidget(), SizedBox(width: 8.w)],
                          Icon(AppIcons.detailView, color: AppColors.opacity40White, size: 18.w),
                        ],
                      ),
                    ),
                  ),
                    ...
                    enabledOnPressed: () async {
                      ...
                      try {
                        ...
                        final itemRequest = ItemRequest(
                          ...
                          latitude: _latitude,
                          isAiPredictedPrice: _isAiRecommendedPrice,
                        );
