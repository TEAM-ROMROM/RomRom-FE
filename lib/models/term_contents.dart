...
class TermContents {
  ...
  const TermContents({required this.title, required this.content});

  // JSON에서 TermContents 생성
  factory TermContents.fromJson(Map<String, dynamic> json) {
    return TermContents(title: json['title'] as String, content: json['content'] as String);
  }
  ...
  static Future<TermContents> _loadSingle(TermsType termType) async {
    try {
      final jsonString = await rootBundle.loadString('assets/terms/${termType.contentKey}.json');
      final jsonData = jsonDecode(jsonString) as Map<String, dynamic>;
      return TermContents.fromJson(jsonData);
    } catch (e) {
      // 로드 실패시 기본값 반환
      return TermContents(title: termType.title, content: '약관을 불러오는 중 오류가 발생했습니다.');
    }
  }
