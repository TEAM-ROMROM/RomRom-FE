...
class AppColors {
  ...
  static const Color opacity30PrimaryBlack = Color(
    0x4D1D1E27,
  ); // 30% 불투명도 primaryBlack
  static const Color opacity90PrimaryBlack = Color(
    0xE61D1E27,
  ); // 90% 불투명도 primaryBlack
  ...
  // 동네 인증하기 화면
  static const Color locationVerificationAreaLabel = Color(
    0x4CFFFFFF,
  ); // 흰색, opacity 30%
  static const Color currentLocationButtonIcon = Color(
    0xFF007AFF,
  ); // 현재 위치 버튼 아이콘
  static const Color currentLocationButtonBg = Color(0xFFFFFFFF); // 현재 위치 버튼 배경
  static const Color currentLocationButtonShadow = Color(
    0xFF001B60,
  ); // 현재 위치 버튼 그림자
  static const Color currentLocationButtonBorder = Color(
    0xFFB7B7B7,
  ); // 현재 위치 버튼 테두리

  static const Color bottomNavigationDisableIcon = Color(
    0xFF676767,
  ); // 하단 네비게이션바 비활성화 아이콘

  // 물품 카드
  static final Color itemCardBackground = const Color(
    0xFFFFFFFF,
  ).withValues(alpha: 0.8); // 물품 카드 배경 색상
  static final Color itemCardBorder = const Color(
    0xFFFFFFFF,
  ).withValues(alpha: 0.6); // 물품 카드 테두리 색상
  static const Color itemCardShadow = Color(
    0x26000000,
  ); // 물품 카드 그림자 색상 , 검정색, opacity 15%
  ...
  // Item 내부 태그
  static const Color conditionTagBackground = Color(
    0xFFFFF1C4,
  ); // 사용감 태그 (노란색 계열)
  static const Color transactionTagBackground = Color(
    0xFFCACDFF,
  ); // 거래 방식 태그 (보라색)
  ...
  // 온보딩 프로그레스 헤더
  static final Color onboardingProgressInactiveLine = primaryYellow.withValues(
    alpha: 0.1,
  ); // 비활성화 프로그레스 선
  static const Color onboardingProgressStepPendingBg =
      primaryBlack; // 대기 단계 배경색 (primaryBlack과 동일)
  static final Color onboardingProgressStepPendingBorder = primaryYellow
      .withValues(alpha: 0.1); // 대기 단계 테두리
  static final Color onboardingProgressStepPendingText = primaryYellow
      .withValues(alpha: 0.3); // 대기 단계 텍스트
  ...
  // 다이얼로그 배리어 (배경 오버레이)
  static const Color dialogBarrier = Color(
    0x80000000,
  ); // 50% 불투명도 검정 (다이얼로그 배경)

  // 물품 등록 화면
  // 물품 교환 AI 추천 가격 토글 Switch
  static const Color toggleSwitchIndicatorShadow = Color(
    0x40000000,
  ); // 검은색, opacity 25%
  static const Color aiSuggestionContainerBackground = Color(
    0x4DCF7DFF,
  ); // ai 추천 가격 태그 배경색
  // 물품 사진
  static const Color itemPictureRemoveButtonBackground = Color(
    0xFFD2D2D2,
  ); // 물품 사진 삭제 버튼 배경색
  ...
  static Color cardDropZoneBackground = Colors.white.withValues(
    alpha: 0.3,
  ); // 드롭 존 배경 색상
  static Color cardDropZoneShadow = Colors.white; // 드롭 존 배경 색상

  // 홈 화면 거래 요청 Dialog
  static const Color transactionRequestDialogCancelButton = Color(
    0xFFDEDACE,
  ); // 거래 요청 다이얼로그 테두리 색상
