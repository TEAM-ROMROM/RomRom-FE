...
### 스크린 파일 구성 및 네이밍
- 스크린 파일명은 `snake_case`로 작성하고 항상 `_screen` 접미사 사용
    - 예: `login_screen.dart`, `home_tab_screen.dart`
- 스크린 클래스명은 `PascalCase`로 작성
    - 예: `LoginScreen`, `HomeTabScreen`
- 중첩 화면은 디렉토리를 만들고 그 안에 관련 화면들을 그룹화
    - 예: `onboarding/term_agreement_step.dart`
...
### 상태 관리 및 로직 분리
- 스크린 위젯은 `StatefulWidget`으로 구현
- 복잡한 상태나 비즈니스 로직은 별도 클래스/서비스로 분리
- 상태 변수는 클래스 상단에 명확한 이름으로 선언
  ...
  ```
- 상태 초기화는 `initState`에서 수행
- 컨텍스트 의존적 초기화는 `didChangeDependencies`에서 수행
...
- 화면별 전용 위젯은 화면 파일 내에서 `_` 접두사를 사용해 정의
  ```dart
...
- 에러 처리는 try-catch 블록 사용
  ...
  ```
- 비동기 상태는 3가지(로딩/성공/에러) 모두 처리
- 비동기 연산 후 화면 업데이트 시 `setState` 호출 전에 위젯이 여전히 트리에 있는지 확인
  ```dart
  if (mounted) {
    setState(() {
      _data = result;
...
### 화면 적응형 처리
- 모든 크기 단위는 `ScreenUtil` 확장 사용 (예: `16.w`, `24.h`)
- 화면 너비/높이는 `MediaQuery`로 동적 계산
  ```dart
...
- 안드로이드 시스템 네비게이션바 처리를 위한 패딩 적용
  ...
  ```
- 모바일 방향 변경 대응은 `OrientationBuilder` 사용 (필요한 경우)

### 성능 최적화
- 무거운 연산은 UI 스레드가 아닌 별도 격리자(isolate)에서 처리
- 대용량 리스트는 `ListView.builder`로 구현
- 불필요한 빌드 방지를 위해 `const` 생성자 최대한 활용
- 이미지는 적절한 크기로 최적화하여 사용
- 복잡한 애니메이션은 `AnimationController`로 최적화
